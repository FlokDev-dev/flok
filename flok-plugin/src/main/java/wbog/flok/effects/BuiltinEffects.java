...
public final class BuiltinEffects {
    ...
    public static void registerAll(EffectRegistry r) {
        ...
        r.register("give", (player, args, ctx) -> {
            if (player == null || args.isEmpty()) return;
            // give <player-ignored> <material> [amount]  OR  give <material> [amount]
            // Scripts typically: give %player-name% diamond 5
            // We ignore the player-name arg since we already have ctx player
            String matName;
            int    amount = 1;
            if (args.size() >= 2) {
        ...
        r.register("potion", (player, args, ctx) -> {
            ...
            // Use Registry lookup (Paper 1.21+); fall back to legacy getByName
        ...
        r.register("scoreboard-line", (player, args, ctx) -> {
            ...
            // scoreboard-line <score> <text>
            // Uses a dummy-criteria scoreboard: each unique text value is a separate entry.
            ...
            if (obj == null) return;
            // Clear existing entry at this score, then set the new one
