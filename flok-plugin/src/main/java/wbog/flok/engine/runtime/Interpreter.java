...
/**
 * AST interpreter for Flok scripts.
 *
 * DESIGN:
 * - Stateless between calls — all mutable state lives in ExecutionContext.
 *   This means the same Interpreter instance can theoretically be reused,
 *   but ScriptEngine constructs one per run() call for simplicity; the object
 *   is tiny (3 final references).
 * - Player variables (%player-health%, %player-x%, etc.) are injected lazily
 *   on first access via injectPlayerVars(). This avoids ~12 FValue allocations
 *   and ~12 Player API calls on every event that never reads them.
 * - Persistent key expressions are evaluated at runtime (strings interpolated)
 *   so __coins-%player-name%__ resolves correctly per player.
 */
public final class Interpreter {
    ...
    public void executeBlock(ASTNode.Block block, ExecutionContext ctx) {
        ...
        for (int i = 0; i < stmts.size(); i++) {
            ...
            try {
                ...
                // Re-throw with the FULL remaining continuation from this block level
                // (the WaitSignal may have been thrown from a nested call; we replace
                // its remaining list with everything still left in THIS block)
    ...
    private void executeStmt(ASTNode node, ExecutionContext ctx) {
        switch (node) {
            ...
            case ASTNode.ExprStmt es -> eval(es.expr(), ctx); // side-effectful call

            case ASTNode.Block b -> executeBlock(b, ctx);

            case ASTNode.WaitStmt ws -> {
                long ticks = Math.max(1L, eval(ws.ticks(), ctx).asLong());
                // Snapshot the current locals so the continuation has them
    ...
    public FValue eval(ASTNode node, ExecutionContext ctx) {
        ...
        return switch (node) {
            ...
            // Property access: map key lookup, or NULL for unsupported types
    ...
    private FValue callFunction(ASTNode.FunctionCall fc, ExecutionContext ctx) {
        // 1. Check script-defined functions
    ...
    private FValue callScriptFunction(ASTNode.FunctionDef def, List<ASTNode> argExprs, ExecutionContext ctx) {
        ctx.pushCall();
        // Build a child context with fresh locals for the function frame
        ...
        ctx.popCall();
        // Sync ops consumed by child back to parent so the total budget is shared
    ...
    private FValue resolveVar(String name, ExecutionContext ctx) {
        ...
        // Lazy player variable injection on first miss of a player-* variable
    ...
    // ── Package-visible for BuiltinFunctions ──────────────────────────────────
